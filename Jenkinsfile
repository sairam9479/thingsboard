pipeline {
	...
	stages {
		...
		stage ('docker build & push') {
			steps{
                sh 'mvn versions:set -DnewVersion=2.5.${BUILD_NUMBER}'
				sh "mvn clean pre-integration-test -DskipTests -Dlicense.skip -Ppush-docker-image -Ddockerfile.skip=false -Ddocker.repo=localhost:5000/corinexgv:2.5.${BUILD_NUMBER}"
            }
		}
